...
void TStats::ClearCommHist()
{
chist.erase(chist.begin(), chist.end());
/*
for (unsigned int i=0; i<chist.size(); i++)
    {
     chist[i].delays.erase(chist[i].delays.begin(), chist[i].delays.end());
     chist[i].total_received_flits=0;
     chist[i].last_received_flit_time=0;
    } // */
