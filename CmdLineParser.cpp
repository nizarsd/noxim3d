...
void showHelp(char selfname[])
{
  ...
  cout << "\t-dimz N\t\tSet the mesh Z dimension to the specified integer value (default " << DEFAULT_MESH_DIM_Z << ")" << endl;
  ...
  cout << "\t\txyz\t\tXYZ routing algorithm" << endl;
...
if (TGlobalParams::verbose_mode!=-2){
  ...
  cout << "%- no. of samples to apply for the thermal model = " << TGlobalParams::no_of_samples<<endl ;
  cout << "%- lower temerature limit C = " << TGlobalParams::t_lower<<endl ; //<Nizar>
  cout << "%- upper temerature limit C = " << TGlobalParams::t_upper<<endl;
...
void checkInputParameters()
{
  ...
  {
    if (TGlobalParams::hotspots[i].first >= TGlobalParams::mesh_dim_x*TGlobalParams::mesh_dim_y*TGlobalParams::mesh_dim_z)
    {
...
void parseCmdLine(int arg_num, char *arg_vet[])
{
  ...
  else
  {
	...
	TGlobalParams::verbose_mode = atoi(arg_vet[++i]);
	 ...
	 TGlobalParams::mesh_dim_z = atoi(arg_vet[++i]);
	...
	else if (!strcmp(routing, "negativefirstnm")) 
	  TGlobalParams::routing_algorithm = ROUTING_NEGATIVE_FIRST_NM;
        ...
        else if (!strcmp(routing, "table"))
	{
        ...
        else if (!strcmp(routing, "dw_xy")) 
	  TGlobalParams::routing_algorithm = ROUTING_DW_XYZ;
        else
	...
	char *selection = arg_vet[++i];
	 ...
	 else if(!strcmp(selection, "dp")) 
	  TGlobalParams::selection_strategy = SEL_DP;
        else 
	...
	TGlobalParams::simulation_time = atoi(arg_vet[++i]);
      else if (!strcmp(arg_vet[i], "-samp"))
	TGlobalParams::no_of_samples = atoi(arg_vet[++i]);
      else if (!strcmp(arg_vet[i], "-tupper"))   // Nizar
	TGlobalParams::t_upper = atoi(arg_vet[++i]);
      else if (!strcmp(arg_vet[i], "-tlower"))   // Nizar
	TGlobalParams::t_lower = atoi(arg_vet[++i]);
      else if (!strcmp(arg_vet[i], "-tmode"))   // Nizar
